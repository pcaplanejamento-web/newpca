...
/** A trava de UM protocolo (ou `null`). */
export async function travaDoProtocolo(id: number | null | undefined): Promise<TravaPca | null> {
  return id ? ((await travaDeProtocolos([id])).get(id) ?? null) : null;
}
