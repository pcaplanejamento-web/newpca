...
import { IconAlert, IconClock, IconRefresh, IconSpinner } from "./icons";
...
/**
 * Banner do PROTOCOLO GRAVADO — os MESMOS componentes e a MESMA conferência da análise (protocolação):
 * corpo `ProtocoloView` (mini banners, conciliação da capa com "Substituir pela somatória", capa com
 * cadeado por campo, planilha de DFDs com seleção), barra de EDIÇÃO EM MASSA, DFD ao lado
 * (`DfdConferir` + `DfdRodape`) e o painel da direita (mensagens/item/histórico), relatório de erros.
 * A ÚNICA diferença: a planilha é UMA tabela só (`unica`). As edições ficam num RASCUNHO até
 * "Salvar alterações" (só o que mudou vai ao banco — `diffCapaGravada`/`diffDfdGravado`).
 */
export function ProtocoloGravado({
  protocoloId,
  dfdInicial = null,
  onClose,
  podeEditar,
  ...
  /** Abre já com este DFD ao lado (ex.: "Ver protocolo" a partir de um DFD). */
  ...
  function fechar() {
    if (salvando) return;
    if (sujo && !confirm("Há alterações não salvas neste protocolo. Fechar e descartá-las?")) return;
    fecharDfd();
    onClose();
  }
  function atualizar() {
      ...
      <Modal
        ...
        lateral={
          ordem.length === 0
            ? undefined
            : {
                aberto: abertoId != null && !!dfdAberto,
        ...
        lateral2={
          ordem.length === 0
            ? undefined
            : {
                aberto: abertoId != null && !!dfdAberto && painel != null,
