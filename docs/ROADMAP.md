...
### Importâncias e estados gerenciáveis pelo ADM (cores + chaves) — entregue
✅ O antigo **nível fixo** (fundamental/intermediário/automático/ignorar) virou uma **lista de IMPORTÂNCIAS gerenciável** pela aba **Avaliação** (`AvaliacaoAdmin`, sub-aba "Importâncias"): o ADM **cria, edita e exclui** importâncias, cada uma com **nome**, **cor** (`ColorField`, hex livre) e um **comportamento** — `bloqueia` (trava import/protocolação, via **`Switch`** "Bloqueia importação/protocolação"), `avisa`, `automatico` ou `ignora` (`Segmented`). As **4 base** (ids == as strings de nível históricas, p/ a config gravada seguir valendo **sem migração**) têm nome/cor editáveis mas comportamento fixo e não são excluíveis; as customizadas têm CRUD total (↑/↓ ordem, `Modal` editor, excluir com `confirm`). Cada ponto de Protocolo/DFD/Item **referencia uma importância** cujo comportamento ∈ `comportamentosPermitidos` do ponto. **As cores SEGUEM a importância** de ponta a ponta: `mensagensDfd`/`resumoEstado` pintam a célula "Estado" e o painel `MensagensDfd` com a cor exata; `estadoCor`/`estadoItemCor`/`estadoProtocoloCor`/`estadoRotulo` recebem `regras` e puxam a cor da importância (severidade) ou dos **estados de ciclo** editáveis (`estadosCiclo`: Editado/Regularizado/Regular/Pendente — nome/cor, quantidade fixa). Novo componente **`Switch`** (design-system, `role="switch"`, alvo ≥44px, catalogado) — a "chave" reusada no "Bloqueia" e no "Editável pelo usuário". A engine foi reescrita **mecanicamente** (`comportamentoNo(regras, chave, ctx)` no lugar de `=== "fundamental"` etc.); `nivelDe` devolve o **id** da importância e o guard descarta ids órfãos (deletar/renomear **nunca corrompe**). Schema (`importanciaSchema`/`estadoCicloSchema` em `avaliacao-validation.ts`) valida id kebab + cor hex + comportamento e o id de cada ponto contra os permitidos. **Invariante:** config vazia ⇒ importâncias/cores/comportamento **idênticos a hoje** (coberto por testes). Sem migração (chaves novas `importancias`/`estadosCiclo` no blob `avaliacao`).
...
ou **Importação** (várias planilhas + cards) e **Configuração**. (A camada por situação foi retirada depois — ver
"Sequencial do item no PCA".) Orçamento ganhou **Visões salvas** (filtro por vários valores de cada dimensão). Migração `0033`
...
**"Devolver à Mesa"** enquanto não incorporados). Na Mesa do PCA, a seleção **incorpora** os protocolos (DFDs + itens passam a
compor o PCA, com a ação incorporar/substituir/excluir) — **permanente**. Incorporado, protocolo/DFDs/itens ficam **somente
leitura** (servidor responde 423; a tela mostra o aviso) — só situação e responsável seguem editáveis. Migração `0034`.

### Sequencial do item no PCA, Dashboard único e situação fora do PCA — entregue
Ao incorporar, cada item ganha um **nº sequencial único no PCA** (gravado também no item); **retirar o item do PCA** só
inativa o número, que nunca é reaproveitado (coluna "Seq. PCA" na Mesa do PCA; o Dashboard mostra o nº). O Dashboard/
...
✅ **Fase 2 (migração `0043`):** **checklist** (progresso no cartão), **comentários com @menção**, **anexos** (links e
imagem/PDF até 1 MB — a imagem maior é reduzida no navegador), **estimativa** e **observadores**, **vínculo** com
